...
async def login(request):
    ...
    if 'korisnicko_ime' not in data or 'lozinka' not in data:
        return web.json_response({'authorized': False, 'message': 'Nedostaju podaci'}, status=400)
    
    korisnik = next((k for k in korisnici if k['korisnicko_ime'] == data['korisnicko_ime']), None)
    
    if not korisnik:
        return web.json_response({'authorized': False, 'message': 'Korisnik ne postoji'}, status=404)
    
    if korisnik['lozinka_hash'] != hash_data(data['lozinka']):
        return web.json_response({'authorized': False, 'message': 'Pogrešna lozinka'}, status=401)
    
    return web.json_response({'authorized': True, 'message': 'Uspješna prijava'}, status=200)
